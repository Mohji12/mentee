...
const WorkingCommitteeStudents = () => {
  ...
  const fetchStudents = () => {
    ...
    const url = `${API_BASE_URL}/working-committee/${member_id}/students${params.toString() ? '?' + params.toString() : ''}`;
  ...
  useEffect(() => {
    fetchStudents();
  }, [member_id, department, mentorId]);

  const mentorsInDepartment = department
  ...
  const submitAssignMentor = () => {
    ...
    fetch(`${API_BASE_URL}/working-committee/${member_id}/students/${student.usn}/mentor`, {
      method: 'PATCH',
      headers: {
  ...
  return (
    <div className="admin-dashboard__main-content leader-dashboard">
      <header className="leader-dashboard__header">
        <h1 className="leader-dashboard__title">Students</h1>
        <p className="leader-dashboard__subtitle">Students in your allocated departments. Use filters to narrow by department or mentor.</p>
      </header>
      ...
      {mentorModal.open && mentorModal.student && (
        <div className="leader-modal-overlay" onClick={closeMentorModal} role="dialog" aria-modal="true" aria-labelledby="wc-assign-mentor-title">
          <div className="leader-modal" onClick={(e) => e.stopPropagation()}>
            <h2 id="wc-assign-mentor-title">Assign mentor – {mentorModal.student.name} ({mentorModal.student.usn})</h2>
            <div className="leader-modal__field">
              <label htmlFor="wc-mentor-select">Mentor</label>
              <select
                id="wc-mentor-select"
...
export default WorkingCommitteeStudents;
