...
const ProgramFacultyMentors = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/program-faculty/${member_id}/filters`, {
      ...
      .then((res) => res.ok ? res.json() : Promise.reject(new Error('Failed to load filters')))
  ...
  const fetchMentors = () => {
    setLoading(true);
    const token = sessionStorage.getItem('access_token');
    fetch(`${API_BASE_URL}/program-faculty/${member_id}/mentors`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then((res) => {
  ...
  const handleCreateSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/program-faculty/${member_id}/mentors`, {
        method: 'POST',
      ...
      if (!res.ok) throw new Error(data.detail || data.message || `Failed to create mentor (${res.status})`);
  ...
  const handleDelete = async (mentorId, studentCount) => {
    const message = studentCount > 0
      ? `Are you sure you want to delete mentor "${mentorId}"?\n\nThis mentor has ${studentCount} assigned student(s). They will be unassigned.\n\nThis action cannot be undone.`
      : `Are you sure you want to delete mentor "${mentorId}"?\n\nThis action cannot be undone.`;
    if (!window.confirm(message)) return;
    setDeletingId(mentorId);
    const token = sessionStorage.getItem('access_token');
    try {
      const res = await fetch(`${API_BASE_URL}/program-faculty/${member_id}/mentors/${mentorId}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <div className="admin-dashboard__main-content leader-dashboard">
      <header className="leader-dashboard__header">
        <h1 className="leader-dashboard__title">Mentors</h1>
        <p className="leader-dashboard__subtitle">Mentors in your allocated program departments. Create and delete mentors here.</p>
      </header>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div className="leader-count">
          ...
          <span>mentor{mentors.length !== 1 ? 's' : ''}</span>
        </div>
        <button
          type="button"
          onClick={(e) => { e.preventDefault(); e.stopPropagation(); setShowCreateModal(true); }}
          disabled={loading}
          style={{
            ...
            border: `1px solid ${loading ? '#94a3b8' : '#3b82f6'}`,
            borderRadius: '8px',
          ...
          <div className="leader-table-wrapper">
            {mentors.length === 0 ? (
              <div className="leader-empty">
                ...
                <p>No mentors in your allocated program departments yet. Create one to get started.</p>
              </div>
            ) : (
            ...
            <div style={{ padding: '1.5rem' }}>
              ...
              <form onSubmit={handleCreateSubmit}>
                <div style={{ marginBottom: '1rem' }}>
                  <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: 600, fontSize: '0.875rem' }}>Mentor ID *</label>
                  <input
                    type="text"
                    required
                    value={createForm.mentor_id}
                    onChange={(e) => setCreateForm({ ...createForm, mentor_id: e.target.value })}
                    style={{ width: '100%', padding: '0.5rem', border: '1px solid #e2e8f0', borderRadius: '8px', fontSize: '0.875rem' }}
                ...
                <div style={{ marginBottom: '1rem' }}>
                  <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: 600, fontSize: '0.875rem' }}>Department *</label>
                  <select
                    required
                    value={createForm.mentor_department}
                    onChange={(e) => setCreateForm({ ...createForm, mentor_department: e.target.value })}
                    style={{ width: '100%', padding: '0.5rem', border: '1px solid #e2e8f0', borderRadius: '8px', fontSize: '0.875rem', backgroundColor: '#fff' }}
                  >
                ...
                <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'flex-end' }}>
                  <button type="button" onClick={() => setShowCreateModal(false)} disabled={createLoading} className="leader-filters__btn-clear">Cancel</button>
                  <button
                    type="submit"
                    disabled={createLoading}
                    style={{
...
export default ProgramFacultyMentors;
