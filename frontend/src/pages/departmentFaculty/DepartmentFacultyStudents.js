...
const DepartmentFacultyStudents = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/department-faculty/${member_id}/filters`, {
  ...
  const fetchStudents = () => {
    ...
    const url = `${API_BASE_URL}/department-faculty/${member_id}/students${params.toString() ? '?' + params.toString() : ''}`;
  ...
  const submitAssignMentor = () => {
    ...
    fetch(`${API_BASE_URL}/department-faculty/${member_id}/students/${student.usn}/mentor`, {
      method: 'PATCH',
      headers: {
  ...
  return (
    <div className="admin-dashboard__main-content leader-dashboard">
      ...
      {mentorModal.open && mentorModal.student && (
        <div className="leader-modal-overlay" onClick={closeMentorModal} role="dialog" aria-modal="true" aria-labelledby="df-assign-mentor-title">
          <div className="leader-modal" onClick={(e) => e.stopPropagation()}>
            <h2 id="df-assign-mentor-title">Assign mentor – {mentorModal.student.name} ({mentorModal.student.usn})</h2>
            <div className="leader-modal__field">
              <label htmlFor="df-mentor-select">Mentor</label>
              <select
                id="df-mentor-select"
...
export default DepartmentFacultyStudents;
