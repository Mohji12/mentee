...
def create_radar_chart(data_dict):
    ...
    try:
        ...
        # Fallback: try without specifying engine, or use orca if available
        try:
            img_bytes = pio.to_image(fig, format="png", width=400, height=400, scale=2)
        except Exception as fallback_error:
            # If all else fails, create a simple placeholder
            print(f"Warning: Could not generate radar chart image: {e}. Using fallback.")
            # Return an empty buffer - the PDF will continue without the chart
            return BytesIO()
